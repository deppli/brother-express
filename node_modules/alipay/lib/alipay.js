...
Alipay.prototype.create_direct_pay_by_user_notify = function(req, res){
	...
	alipayNotify.verifyNotify(_POST, function(verify_result){
		if(verify_result) {//验证成功
			...
			var params = {};
			params.order_no = _GET['out_trade_no'];
			params.alipay_no = _GET['trade_no']
			params.remark = _GET['subject']
			params.amount = _GET['total_fee']

			if(trade_status  == 'TRADE_FINISHED'){
				self.emit('create_direct_pay_by_user_trade_finished', params);
			}
			else if(trade_status == 'TRADE_SUCCESS'){
				self.emit('create_direct_pay_by_user_trade_success', params);
			}

			//res.send("success");		//请不要修改或删除
			res.render("views/direct_success")

		}else {
			//验证失败
			self.emit("verify_fail");
			res.render("views/verify_fail")
			//res.send("fail");
		}
	});	
}
...
Alipay.prototype.create_direct_pay_by_user_return = function(req, res){		
	...
	alipayNotify.verifyReturn(_GET, function(verify_result){
		if(verify_result) {//验证成功
			...
			if(trade_status  == 'TRADE_FINISHED'){                
				self.emit('create_direct_pay_by_user_trade_finished', req, params);
			}
			else if(trade_status == 'TRADE_SUCCESS'){                
				self.emit('create_direct_pay_by_user_trade_success', req, params);
			}
